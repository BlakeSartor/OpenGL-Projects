...
// Weight funciton used to calculate h
float Ground::w(float d_) {
    float d_over_5 = d_/5;
    float sq = d_over_5 * d_over_5;
    return fmax(0,1-sq);
}
                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    Point2 norm_sp_start = Point2(stroke2d[0][0],stroke2d[0][1]);
    Point2 norm_sp_end = Point2(stroke2d[stroke2d.size()-1][0],stroke2d[stroke2d.size()-1][1]);
    
    // 2D starting point to 3D ground point
    ScreenPtToGround(view_matrix, proj_matrix, norm_sp_start, &p_start);
    // 2D ending point to 3D ground point
    ScreenPtToGround(view_matrix, proj_matrix, norm_sp_end, &p_end);
    ...
    for (int i=0; i < stroke2d.size(); i++) {
        Point3 plane_point;
        Point2 norm_sp = Point2(stroke2d[i][0],stroke2d[i][1]);
        // Screen point to plane defined above
        Point3 pt3d = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, norm_sp);
        Ray ray(eye, (pt3d - eye).ToUnit());
        float t;
        ray.IntersectPlane(p_start, plane_normal, &t, &plane_point);
        // store plane_point into verts
        verts.push_back(plane_point);
        }
    ...
    // for each point in the ground mesh
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        Point3 P = ground_mesh_.vertex(i); // original vertex
        // Closest point to P that lies within plane defined above
        Point3 p_in_plane = P.ClosestPointOnPlane(p_start, plane_normal);
        // Calulate h from plane and closet point to P within the plane
        float h = hfunc(plane_normal, verts, p_in_plane);
        // Distance from P to plane
        float d = P.DistanceToPlane(p_start, plane_normal);
